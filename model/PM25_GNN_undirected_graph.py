...
class GraphGNN(nn.Module):
    def __init__(self, device, edge_index, edge_attr, in_dim, out_dim, wind_mean, wind_std):
        ...
        e_out = 30
        n_out = out_dim
        self.edge_mlp = Sequential(Linear(in_dim * 2 + 2 , e_h),
                                   Sigmoid(),
                                   Linear(e_h, e_out),
                                   Sigmoid(),
                                   )
    ...
    def forward(self, x):
        ...
        self.edge_attr_ = self.edge_attr[None, :, None].repeat(node_src.size(0), 1, 1)
        city_dist = self.edge_attr_[:,:,0]

        # Adjusted the edge_weight calculation to not depend on city direction
        edge_weight = F.relu(3 * src_wind_speed / city_dist)
        edge_weight = edge_weight.to(self.device)

        edge_attr_norm = self.edge_attr_norm[None, :, None].repeat(node_src.size(0), 1, 1).to(self.device)
