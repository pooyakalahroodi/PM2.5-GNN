...
class GC_Transformer(nn.Module):
    ...
    # Create a mask for a single batch
        single_mask = torch.full((num_cities * time_steps, num_cities * time_steps), float('-inf'))
   
        for i in range(num_cities):
              start_idx = i * time_steps
              end_idx = start_idx + time_steps
              single_mask[start_idx:end_idx, start_idx:end_idx] = 0

              # Repeat the mask for each batch
        mask = single_mask.unsqueeze(0).repeat(batch_size, 1, 1)
        sreturn mask
