...
class Transformer_with_PE(nn.Module):
    ...
    def create_causal_mask(self, num_heads, num_cities, batch_size, seq_len):
        mask_size = batch_size * seq_len
        mask = torch.triu(torch.ones(mask_size, mask_size) * float('-inf'), diagonal=1)
        mask = mask.repeat(num_heads * num_cities, 1, 1)
        return mask
    ...
    def forward(self, x):
        ...
        causal_mask = self.create_causal_mask(self.num_heads, self.city_num, batch_size, seq_len).to(x.device)
        # Multi-Head Self-Attention
        attn_output, _ = self.self_attention(x_flat, x_flat, x_flat, attn_mask=causal_mask)
