...
class TransformerGNN_with_PE(nn.Module):
    def __init__(self, hist_len, pred_len, in_dim, city_num, batch_size, device, edge_index, edge_attr, wind_mean, wind_std, use_positional_encoding=True):
        ...
        # Adjust the input dimension of fc_out to match the transformer's output
        self.device = device
        ...
        if use_positional_encoding:
            self.transformer = Transformer_with_PE(input_dim=self.in_dim + self.gnn_out, seq_len = hist_len ,hidden_dim=self.hid_dim, num_heads=4, num_layers=4)
        else:
            self.transformer = Transformer_with_PE(input_dim=self.in_dim + self.gnn_out, seq_len = hist_len ,hidden_dim=self.hid_dim, num_heads=4, num_layers=4)
